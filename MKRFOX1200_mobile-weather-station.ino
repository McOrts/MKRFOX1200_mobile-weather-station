...
// Sensor de interno de voltaje
 float BatteryLevel = 0;

// Estructura para almacenar datos
typedef struct __attribute__ ((packed)) sigfox_message {
  // int16_t sigfoxTemperature;
  float mrkBatteryLevel;
...
void loop() {
  ...
  float BatteryLevel = analogRead(ADC_BATTERY) * 3.76 / 400;
  ...
  if (uvPrimera && tempPrimera && empiezaTransmitir)
  {
    ...
    msg.mrkBatteryLevel = BatteryLevel;
...
void medirUV()
{
  ...
  //float outputVoltage = BatteryLevel / refLevel * uvLevel;
